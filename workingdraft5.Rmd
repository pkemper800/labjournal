...
This establishes the dataframes for preliminary analysis.  I will first try analysis of this subset from the "scholars" databased with just professors from Radboud University, before then scaling analysis to all the universities. Notably, this database already includes both sociology and political science disciplines (departments for each respective university). I will analyze gender in collaborations, though it will be important later to control for the discipline. 
...
# Network Descriptives - Radboud University

## Visualizing Networks - Radboud University Waves 1 and 2
...
### RU Wave 1 - Second network visualisation: Closer look at clustering 
``` {r eval=FALSE}
...
V(test_wave1ru)$size = betweenness(test_wave1ru, normalized = T, directed = TRUE) *400 + 10  #after some trial and error
...
# story in second plot: looks like more clustering by gender 
...
### RU Wave 2 - Second network visualisation: Closer look at clustering 
``` {r}
# igraph, want no overlap: igraph plotting no overlap -- a lot of layout functions -- want to hold printing device constant, and then reduce overlap...the idea is to push least central egos out. 
V(test_wave2ru)$size = betweenness(test_wave2ru, normalized = T, directed = TRUE) *400 + 10  #after some trial and error

set.seed(2345)
l <- layout_with_mds(test_wave2ru)  #https://igraph.org/r/doc/layout_with_mds.html
...
# Descriptive Statistics - Radboud Professors 


With this, we are looking at measures of centrality. The number of degrees of connections is the most simple term for centrality. 

We want to describe if this network is complete, the size of this network, and what kind of relationships there are (whether or not it is directed), and the node type (as well as if the tie is weighted).

When looking at degree centrality, if there are differences in degree centrality then we could illustrate nodes by differing size (for example) to accommodate for this. 

We will briefly look at local and global transitivity, looking at both the dyads and triads that form in this collaboration network for reference. Considering this is not a big part of the research questions though, we will not focus on this in the analysis. 
...
Note to self: Could segment by wave?
...
# View summary
table(df_ego$xgen_w1, df_ego$gender, useNA = "ifany")
...
table(df_ego$xgen_w2, df_ego$gender, useNA = "ifany")
 # count number of male and female professors who did and did not participate in x gender collaborations
...
## Looking at Degree and Reciprocity from Ministeps 
```{r}
...
How to answer: Make a 2x2 table, looking at cross-gender collaborations and same-gender collaborations. 

First, we will look at the total number of collaborations across both waves. We will look at the proportion of men who participated in collaborations out of the total number of men. We will look at the total number of women who participated in collaborations out of the total number of women. 

The table will compare the proportion of cross-gender collaborations out of the total number of collaborations, and the proportion of same-gender collaborations out of the total number of collaborations. T

Then we will look at the proportion of women in the cross gender collaborations and the proportion of men in the cross gender collaborations.  
...
Hypothesis: Women are more likely to collaborate than men. 
...
Rate parameters: 
  0       Rate parameter                2.8404  ( 0.5588   )             

Other parameters: 
  1. eval outdegree (density)          -2.2684  ( 0.6274   )    0.1844   
  2. eval reciprocity                   1.6840  ( 0.5093   )   -0.0589   
  3. eval indegree-popularity (centrd)  0.4017  ( 0.0688   )    0.3816   
  4. eval network-isolate               6.1389  ( 1.9186   )   -0.1415   
  5. eval female ego                    1.8541  ( 1.1184   )   -0.1786   
  6. eval polsci ego                   -1.5159  ( 0.7610   )    0.0405   

Overall maximum convergence ratio:    0.3948 
...
Rate parameters: 
  0       Rate parameter                3.2503  ( 0.6470   )             

Other parameters: 
  1. eval outdegree (density)          -2.8290  ( 0.4447   )    0.1208   
  2. eval reciprocity                   1.7031  ( 0.4641   )   -0.1111   
  3. eval indegree-popularity (centrd)  0.3635  ( 0.0654   )    0.2779   
  4. eval network-isolate               5.8642  ( 1.5231   )   -0.0988   
  5. eval female ego                    0.0899  ( 0.7543   )   -0.0581   
  6. eval female difference            -0.2598  ( 0.2696   )   -0.1592   
  7. eval xgen_w2 ego                   1.7724  ( 0.6427   )   -0.0963   
  8. eval female ego x xgen_w2 ego      3.2247  ( 1.4909   )    0.0038   

Overall maximum convergence ratio:    0.3307 



### ansM6 - Include varCovar of both xgen timeframes
```{r}
mydata6 = sienaDataCreate(net, female) 
myeff6 = getEffects(mydata6)
myeff6 = includeEffects(myeff6, isolateNet, inPop.c)

myeff6 = includeEffects(myeff6, egoX, interaction1 = "female")
...
### ansM7 - look at female interaction with wave2 xgen collaborations, controlling for discipline
```{r}
mydata7 = sienaDataCreate(net, female, xgen_w2) 
myeff7 = getEffects(mydata7)
myeff7 = includeEffects(myeff7, isolateNet, inPop.c)

myeff7 = includeEffects(myeff7, egoX, diffX, interaction1 = "female")
...
# The geodesic distribution is not available from within RSiena, and therefore is copied from the help page of sienaGOF-auxiliary:
...
We can see that this model underestimates at 0, overestimates between 1-5, and then evens out. 
FACTOR IN GWESP TO MODERATE EFFECT:

Indegree
Outdegree 

GWESP - triadic closure measure 

```{r}
# IDEALLY NEED TO FIGURE THIS OUT TOO - IsolateNet/GWESP Inclusion
gofi0 <- sienaGOF(ansM1, IndegreeDistribution, isolateNet, verbose = FALSE, join = TRUE, varName = "net")
...
```
Outdegree distribution pretty aligned as is! 
...
### GOF: Indegree - ansM7
```{r}
gofi2 <- sienaGOF(ansM7, IndegreeDistribution, verbose = FALSE, join = TRUE, varName = "net")
plot(gofi2)
```
...
### GOF: Outdegree - ansM7
```{r}
gofi3 <- sienaGOF(ansM7, OutdegreeDistribution, verbose = FALSE, join = TRUE, varName = "net")
plot(gofi3)
```

### Relative - couldn't complete, because M7 has interaction
```{r}
#RI2 <- RSiena:::sienaRI(data = mydata7, ans = ansM7)
#RSiena:::plot.sienaRI(RI2, addPieChart = TRUE)

# sienaRI does not work for models containing interaction effects
```
